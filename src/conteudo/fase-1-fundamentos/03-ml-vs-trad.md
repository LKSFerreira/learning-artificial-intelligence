...
Na lição **Inteligência Artificial (IA), Machine Learning (ML) e Deep Learning (DL) em um Diagrama de Venn** o mapa ficou assentado: **IA** é o guarda-chuva; **Machine Learning** é a fatia que **aprende com dados e experiência**; **Deep Learning** é uma fatia dentro do ML. No círculo de ML você também viu os **três pilares**: supervisionado, não supervisionado e por reforço.
...
Regras explícitas **continuam** sendo IA (como no diagrama de Venn: nem toda IA é ML). O ponto aqui é o **gargalo**: quando as variações multiplicam, treinar com experiência costuma ser o caminho certo.
...
A regra só olha se a cor é `vermelho`. Resultados: **ACERTO** (achou poção de verdade), **ENGANO** (achou poção e não era) e **ERRADO** (não marcou como poção: perdeu uma poção real ou recusou um item que não é poção).
...
Na lição **Machine Learning (ML)** os três pilares do ML foram apresentados em geral. Aqui você **vive** um deles.

**Resposta: aprendizado supervisionado com dados rotulados** (classificação em duas classes: poção / não é poção).

| Tipo (mapa da hierarquia) | O que recebe | Neste painel |
| --- | --- | --- |
...
Quando a diferença entre **regras** e **exemplos rotulados (supervisionado)** estiver clara no simulador, o próximo passo é por que redes **profundas** mudaram o jogo em imagens e linguagem (e como isso se encaixa no diagrama de Venn da hierarquia).
<!-- audio-skip-start -->
### 📚 Referências Científicas & Leituras Recomendadas

*   **1959 (Arthur L. Samuel):** [Some Studies in Machine Learning Using the Game of Checkers](https://doi.org/10.1147/rd.33.0210) (*IBM Journal of Research and Development*): marco clássico do termo *machine learning* (já citado no diagrama de Venn).
